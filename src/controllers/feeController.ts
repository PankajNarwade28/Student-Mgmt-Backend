...
export class FeeController {
  constructor(
    @inject(FeeRepository) private readonly feeRepository: FeeRepository,
  ) {}
  ...
  processPayment = async (req: Request, res: Response) => {
    ...
    try {
      const payment = await this.feeRepository.recordPayment(
        enrollmentId,
        amount,
        "",
        "",
  ...
  // Inside the FeeController class:
  getInvoice = async (req: Request, res: Response) => {
    const { transactionId } = req.params;

    try {
      const txn = await this.feeRepository.getTransactionDetails(
      ...
      // Details Section
      doc.setFontSize(12);
      doc.text(`Receipt ID: ${txn.razorpay_payment_id}`, 20, 40);
      doc.text(
        `Date: ${new Date(txn.payment_date).toLocaleDateString()}`,
        20,
        50,
      );
      doc.text(`Student Email: ${txn.student_email}`, 20, 60);

      autoTable(doc, {
        startY: 70,
        head: [["Description", "Course", "Amount"]],
        body: [
          ["Tuition Fee Payment", txn.course_name, `INR ${txn.amount_paid}`],
        ],
        styles: { fontSize: 10 },
      ...
      doc.text("Status: SUCCESSFUL / PAID", 20, finalY);
      doc.setFontSize(10);
      doc.text(
      ...
      // Inside getInvoice method
      const pdfOutput = doc.output("arraybuffer");

      // 1. Set the exact content type
      res.setHeader("Content-Type", "application/pdf");

      // 2. Suggest the filename and ensure it ends in .pdf
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="Invoice_${txn.student_email}_${txn.course_name}_${transactionId}.pdf"`,
      );
  ...
  // Admin Revenue Stats Endpoint
  getRevenueStats = async (req: Request, res: Response) => {
  try {
    const data = await this.feeRepository.getRevenueData();
    res.status(200).json({ success: true, data });
  } catch (error) {
