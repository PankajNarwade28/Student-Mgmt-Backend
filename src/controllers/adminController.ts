...
export class AdminController {
  // Repository is injected automatically by Inversify
  constructor(@inject(TYPES.AdminRepository) private adminRepo: AdminRepository) {}
...
const existingUser = await this.adminRepo.findByEmail(normalizedEmail);
      ...
      // const isAdmin = await this.adminRepo.findById(req.user.id);
      // if (!isAdmin || isAdmin.role !== 'Admin') {
      //   res.status(403).json({ message: "Unauthorized: Admins only" });
      //   return;
      // }

      const newUser = await this.adminRepo.createUser(email, role || 'Student');
